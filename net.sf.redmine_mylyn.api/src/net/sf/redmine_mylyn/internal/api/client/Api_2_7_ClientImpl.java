...
public class Api_2_7_ClientImpl extends AbstractClient {
	...
	public void updateConfiguration(IProgressMonitor monitor) throws RedmineApiErrorException {
		...
		for (final Entry<String, IModelParser<? extends AbstractPropertyContainer<?>>> entry : parserByClass.entrySet()) {
	...
	public Issue[] query(final Query query, IProgressMonitor monitor) throws RedmineApiErrorException {
		...
		final List<NameValuePair> params = query.getParams();
	...
	@Override
	public void updateIssue(final int issueId, final Map<RedmineApiIssueProperty, String> issueValues, final String comment, final TimeEntry timeEntry, final IRedmineApiErrorCollector errorCollector, IProgressMonitor monitor) throws RedmineApiInvalidDataException, RedmineApiErrorException {
		monitor = Policy.monitorFor(monitor);
		monitor.beginTask(Messages.PROGRESS_UPLOAD_TASK, 1);
